...
public class CtriptHotelListSpiderBiz {
	...
	public void runCity(CtripCityDO cityDO) {
		...
		while(i<=totel) {
			...
			BaseFullResponse<Collection<CtripHotelInfoDO>> res = AjaxCtripCityHotelListUtil.ajaxCityHotelList(currentPage, cityDO.getId());
			if(!res.isSuccess()) {
			...
			if(!CollectionUtils.isEmpty(jsonArray)) {
				...
				jedis.hset(redisCityKey, currentPage+"", "1");
			}else {
